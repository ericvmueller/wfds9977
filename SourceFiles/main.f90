...
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: TC2_LOC,TC2_GLB
...
ALLOCATE(TC2_GLB(2,MAX(N_DEVC,1)),STAT=IZERO)
CALL ChkMemErr('MAIN','TC2_GLB',IZERO)
ALLOCATE(TC2_LOC(2,MAX(N_DEVC,1)),STAT=IZERO)
...
IF_DUMP_DEVC: IF (T>=DEVC_CLOCK .AND. N_DEVC>0) THEN
   ...
   ELSE
      TI_GLB = TI_LOC
   ENDIF
   ! MAXLOC/MINLOC: dump location itself (do not scale by MPI sum of TIME_INTERVAL)
   DO N=1,N_DEVC
      IF (DEVICE(N)%STATISTICS(1:6)=='MAXLOC' .OR. DEVICE(N)%STATISTICS(1:6)=='MINLOC') TI_GLB(N) = 1._EB
   ENDDO
   ...
   IF (MINVAL(TI_GLB)>0._EB) THEN
      TC_LOC(1:N_DEVC) = 0._EB
      DO N=1,N_DEVC
         IF (DEVICE(N)%STATISTICS(1:6)/='MAXLOC' .AND. DEVICE(N)%STATISTICS(1:6)/='MINLOC') THEN
            TC_LOC(N) = DEVICE(N)%VALUE
         ENDIF
      ENDDO
      IF (USE_MPI) THEN
      ...
      ! MAXLOC: reduce by quantity extremum; second entry carries the location
      TC2_LOC = -1.E30_EB
      TC2_LOC(2,:) = 0._EB
      DO N=1,N_DEVC
         IF (DEVICE(N)%STATISTICS(1:6)=='MAXLOC') THEN
            TC2_LOC(1,N) = DEVICE(N)%TEMP_STAT_VALUE
            TC2_LOC(2,N) = DEVICE(N)%VALUE
         ENDIF
      ENDDO
      IF (USE_MPI) THEN
         CALL MPI_ALLREDUCE(TC2_LOC(1,1),TC2_GLB(1,1),N_DEVC,MPI_2DOUBLE_PRECISION,MPI_MAXLOC,MPI_COMM_WORLD,IERR)
      ELSE
         TC2_GLB = TC2_LOC
      ENDIF
      IF (MYID==0) THEN
         DO N=1,N_DEVC
            IF (DEVICE(N)%STATISTICS(1:6)=='MAXLOC') TC_GLB(N) = TC2_GLB(2,N)
         ENDDO
      ENDIF
      ...
      TC2_LOC(2,:) = 0._EB
      DO N=1,N_DEVC
         IF (DEVICE(N)%STATISTICS(1:6)=='MINLOC') THEN
            TC2_LOC(1,N) = DEVICE(N)%TEMP_STAT_VALUE
            TC2_LOC(2,N) = DEVICE(N)%VALUE
         ENDIF
      ENDDO
      IF (USE_MPI) THEN
         CALL MPI_ALLREDUCE(TC2_LOC(1,1),TC2_GLB(1,1),N_DEVC,MPI_2DOUBLE_PRECISION,MPI_MINLOC,MPI_COMM_WORLD,IERR)
      ELSE
         TC2_GLB = TC2_LOC
      ENDIF
      IF (MYID==0) THEN
         DO N=1,N_DEVC
            IF (DEVICE(N)%STATISTICS(1:6)=='MINLOC') TC_GLB(N) = TC2_GLB(2,N)
         ENDDO
      ENDIF
      ...
      DO N=1,N_DEVC
         ...
         IF (DEVICE(N)%STATISTICS(1:6)=='MAXLOC') DEVICE(N)%TEMP_STAT_VALUE = -1.E30_EB
         IF (DEVICE(N)%STATISTICS(1:6)=='MINLOC') DEVICE(N)%TEMP_STAT_VALUE =  1.E30_EB
      ENDDO
   ENDIF
