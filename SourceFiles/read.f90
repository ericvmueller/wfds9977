...
CHARACTER(30) :: QUANTITY,PROP_ID,CTRL_ID,DEVC_ID,SURF_ID,STATISTICS,SPATIAL_STATISTIC,PART_ID,MATL_ID,SPEC_ID,UNITS,&
                 DUCT_ID,NODE_ID(2),X_ID,Y_ID,Z_ID
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                ...
                RELATIVE,ROTATION,SETPOINT,SMOOTHING_FACTOR,SPEC_ID,SPATIAL_STATISTIC,STATISTICS,SURF_ID,TIME_AVERAGED,&
                TRIP_DIRECTION,UNITS,VELO_INDEX,VIRTUAL_INDEX,XB,XYZ,X_ID,Y_ID,Z_ID
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   ! Allow modern SPATIAL_STATISTIC alias for STATISTICS (e.g. MAXLOC X)
   IF (SPATIAL_STATISTIC/='null') THEN
      IF (STATISTICS/='null' .AND. STATISTICS/=SPATIAL_STATISTIC) THEN
         WRITE(MESSAGE,'(A,A,A)') 'ERROR: DEVC ',TRIM(ID),' has conflicting STATISTICS and SPATIAL_STATISTIC'
         CALL SHUTDOWN(MESSAGE)
      ENDIF
      STATISTICS = SPATIAL_STATISTIC
   ENDIF
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      IF (STATISTICS(1:6)=='MAXLOC' .OR. STATISTICS(1:6)=='MINLOC') THEN
         ...
         IF (STATISTICS(1:6)=='MAXLOC') DV%TEMP_STAT_VALUE = -1.E30_EB
         IF (STATISTICS(1:6)=='MINLOC') DV%TEMP_STAT_VALUE =  1.E30_EB
         IF (DV%UNITS=='null') DV%UNITS = 'm'
      ENDIF
   ...
   IF ((XB(1)>-1.E5_EB.OR.STATISTICS/='null') .AND. POINTS==1) THEN
      NM = DV%MESH
      M=>MESHES(NM)
      ! MAXLOC/MINLOC keep the full XB so the search can span multiple meshes
      IF (STATISTICS(1:6)/='MAXLOC' .AND. STATISTICS(1:6)/='MINLOC') THEN
         XB(1) = MAX(XB(1),M%XS)
         XB(2) = MIN(XB(2),M%XF)
         XB(3) = MAX(XB(3),M%YS)
         XB(4) = MIN(XB(4),M%YF)
         ...
         DV%Z2 = XB(6)
      ENDIF
      DV%I1 = NINT( GINV(MAX(DV%X1,M%XS)-M%XS,1,NM)*M%RDXI)
      DV%I2 = NINT( GINV(MIN(DV%X2,M%XF)-M%XS,1,NM)*M%RDXI)
      DV%J1 = NINT( GINV(MAX(DV%Y1,M%YS)-M%YS,2,NM)*M%RDETA)
      DV%J2 = NINT( GINV(MIN(DV%Y2,M%YF)-M%YS,2,NM)*M%RDETA)
      DV%K1 = NINT( GINV(MAX(DV%Z1,M%ZS)-M%ZS,3,NM)*M%RDZETA)
      DV%K2 = NINT( GINV(MIN(DV%Z2,M%ZF)-M%ZS,3,NM)*M%RDZETA)
      IF (DV%I1<DV%I2) DV%I1 = DV%I1 + 1
      IF (DV%J1<DV%J2) DV%J1 = DV%J1 + 1
      IF (DV%K1<DV%K2) DV%K1 = DV%K1 + 1
      IF (ABS(DV%X1-DV%X2)<=SPACING(DV%X2) .AND. IOR==0) DV%IOR = 1
      IF (ABS(DV%Y1-DV%Y2)<=SPACING(DV%Y2) .AND. IOR==0) DV%IOR = 2
      IF (ABS(DV%Z1-DV%Z2)<=SPACING(DV%Z2) .AND. IOR==0) DV%IOR = 3
   ENDIF
