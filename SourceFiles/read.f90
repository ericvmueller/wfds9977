...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,BAROCLINIC,BNDF_DEFAULT,&
                ...
                THERMAL_ELEMENTS,THERMOPHORETIC_DEPOSITION,&
                ...
                VEG_LEVEL_SET_LS_COMPUTE_TIME,VEG_LEVEL_SET_CFD_COMPUTE_TIME, &
...
THERMAL_ELEMENTS               = .FALSE. !thermal elements from an HRRPUA burner, no level set
...
! Thermal elements driven by a fixed HRRPUA burner, with no level set fire spread model.
! VEG_LEVEL_SET_THERMAL_ELEMENTS obtains the thermal element heat content from the level set
! surface heat flux and can therefore only be used with the level set model. THERMAL_ELEMENTS
! obtains it from SF%HRRPUA instead, so the two are mutually exclusive.

IF (THERMAL_ELEMENTS .AND. VEG_LEVEL_SET_THERMAL_ELEMENTS) THEN
  WRITE(MESSAGE,'(A)') 'ERROR: On MISC, THERMAL_ELEMENTS and VEG_LEVEL_SET_THERMAL_ELEMENTS cannot both be .TRUE.'
  CALL SHUTDOWN(MESSAGE)
ENDIF

IF (THERMAL_ELEMENTS .AND. (VEG_LEVEL_SET_COUPLED .OR. VEG_LEVEL_SET_UNCOUPLED)) THEN
  WRITE(MESSAGE,'(A)') 'ERROR: On MISC, THERMAL_ELEMENTS is for use without the level set model. ' // &
                       'Use VEG_LEVEL_SET_THERMAL_ELEMENTS instead.'
  CALL SHUTDOWN(MESSAGE)
ENDIF
...
INTEGER :: INT_LABEL,N_CFCR_TREES,NDIM,N_IGNITORS,N_NON_IN_FILE_TREES,CSTAT
...
LOGICAL  :: LABEL_WITH_CHARACTERS,LABEL_ONLY_NUMBERS
...
COUNT_VEG_LOOP: DO
   ...
   LABEL_WITH_CHARACTERS = .FALSE.
   LABEL_ONLY_NUMBERS   = .FALSE.
   ...
   READ(LABEL,*,IOSTAT=CSTAT) INT_LABEL
   IF (OUTPUT_TREE .AND. LABEL /= 'null' .AND. CSTAT /= 0) THEN !LABEL with characters
     LABEL_WITH_CHARACTERS = .TRUE.
     N_TREES_OUTPUT_DATA = N_TREES_OUTPUT_DATA + 1 !c
   ENDIF
   IF (OUTPUT_TREE .AND. LABEL /= 'null' .AND. CSTAT == 0) THEN !number only LABEL needed when multiple &TREE for same LABEL
     LABEL_ONLY_NUMBERS   = .TRUE.
     IF (INT_LABEL > N_TREES_OUTPUT_DATA) N_TREES_OUTPUT_DATA = INT_LABEL 
   ENDIF
   IF (LABEL_WITH_CHARACTERS .AND. LABEL_ONLY_NUMBERS) THEN
     WRITE(MESSAGE,'(A)') 'ERROR with LABEL in &TREE: all number LABEL cannot be used with other LABELs with characters'
     CALL SHUTDOWN(MESSAGE)
   ENDIF
   ...
   ENDIF
   12 IF (IOS>0) THEN 
         WRITE(MESSAGE,'(A,I10)') 'ERROR: Problem with TREE line ',N_TREES+1
         CALL SHUTDOWN(MESSAGE)
      ENDIF
...
IF (NDIM > 0) THEN
 ...
 ALLOCATE(VOLFRAC_RECT_VEG(NDIM),STAT=IZERO)
 CALL ChkMemErr('READ','VOLFRAC_RECT_VEG',IZERO)
ENDIF
...
!
   ...
   IF (OUTPUT_TREE .AND. LABEL=='null') THEN 
         WRITE(MESSAGE,'(A,I6)') 'ERROR: OUTPUT_TREE=T with no LABEL in TREE namelist ',N
         CALL SHUTDOWN(MESSAGE)
   ENDIF
...
! Identify the number of the tree among all trees with OUTPUT_TREE = T that should have tree average stats output
   ...
   ENDIF
!number only LABEL needed when multiple &TREE for same LABEL.
!it's assumed that the tree number (LABEL) increases monotonically as the input file is read
   IF (OUTPUT_TREE .AND. LABEL /= 'null' .AND. CSTAT == 0) THEN 
     IF (INT_LABEL > N_TREES_OUTPUT_DATA) N_TREES_OUTPUT_DATA = INT_LABEL 
     N_TREE_OUT(N) = N_TREES_OUTPUT_DATA
   ENDIF
...
!
   IF (FUEL_GEOM=='RECTANGLE') THEN
     ...
     VOLFRAC_RECT_VEG(N_RECTANGLE_TREES) = 1._EB
     IF (IGNITOR_ELEMENTS) THEN
      N_IGNITORS                      = N_IGNITORS + 1
      TREE_IGN_INDEX(N)               = N_IGNITORS
      TON_IGN_ELEMS(N_IGNITORS)       = TON_IGNITOR_ELEMENTS
...
!  The level set fuel energy accounting (and fireline intensity output) use SF%VEG_CHAR_FRACTION.
!  Historically VEG_LSET_CHAR_FRACTION was read from the SURF line but never used. If it is
!  specified, honor it by overriding VEG_CHAR_FRACTION on this SURF; otherwise it inherits
!  VEG_CHAR_FRACTION so the two are always consistent.
   IF (VEG_LSET_CHAR_FRACTION >= 0._EB) THEN
      SF%VEG_LSET_CHAR_FRACTION = VEG_LSET_CHAR_FRACTION
      SF%VEG_CHAR_FRACTION      = VEG_LSET_CHAR_FRACTION
   ELSE
      SF%VEG_LSET_CHAR_FRACTION = VEG_CHAR_FRACTION
   ENDIF
...
VEG_LSET_CHAR_FRACTION       = -1._EB !sentinel: <0 means not specified; inherits VEG_CHAR_FRACTION (0.2 default)
...
!-- Thermal element burner (MISC THERMAL_ELEMENTS=.TRUE., no level set model). A SURF that has
!   both HRRPUA and a PART_ID is taken to be a burner whose heat is carried into the gas phase by
!   the thermal elements it emits (see WALL_PARTICLE_INSERT in part.f90 and
!   THERMAL_ELEMENT_HEAT_RELEASE in vege.f90), not by gas phase combustion of a fuel mass flux.
!   BURNING is therefore switched off so that (a) no fuel mass flux is assigned from HRRPUA
!   further below, which would double count the energy, and (b) the surface is allowed to have the
!   small blowing velocity (VEL<0) that WALL_PARTICLE_INSERT requires in order to emit particles.
!   Unlike the level set implementation, this does not require REAC HEAT_OF_COMBUSTION=0.

   IF (THERMAL_ELEMENTS .AND. SF%HRRPUA > 0.0_EB .AND. SF%PART_INDEX > 0) THEN
      SF%THERMAL_ELEMENT_BURNER = .TRUE.
      BURNING = .FALSE.
   ENDIF
