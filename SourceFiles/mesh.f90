...
!  REAL(EB), POINTER, DIMENSION(:,:,:,:) :: DMPVDT_FM_VEG
   ...
   ! Level set per-cell burn progress and energy budget (see LEVEL_SET_FIREFRONT_PROPAGATION):
   ! XLEAD_LS, XTRAIL_LS = leading (fire front) and trailing (burnout line) edge positions across
   ! the grid cell, nondimensionalized 0 to 1; E_REL_LS = energy per unit area released so far,
   ! J/m^2; E_AVAIL_LS = energy per unit area available at ignition (<0 before ignition), J/m^2
   REAL(EB), POINTER, DIMENSION(:,:) :: XLEAD_LS,XTRAIL_LS,E_REL_LS,E_AVAIL_LS
...
XLEAD_LS      => M%XLEAD_LS
XTRAIL_LS     => M%XTRAIL_LS
E_REL_LS      => M%E_REL_LS
E_AVAIL_LS    => M%E_AVAIL_LS
DZTDX         => M%DZTDX
DZTDY         => M%DZTDY
MAG_ZT        => M%MAG_ZT
ASPECT        => M%ASPECT
